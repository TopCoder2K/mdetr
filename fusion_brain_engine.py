...
def go_through_epoch(args, model, criterion, contrastive_criterion, qa_criterion, batch, device, return_staff=False):
    samples = batch["samples"].to(device)
    positive_map = batch["positive_map"].to(device) if "positive_map" in batch else None
    targets = batch["targets"]
    answers = {k: v.to(device) for k, v in batch["answers"].items()} if "answers" in batch else None
    ...
    if return_staff:
        return loss_dict, targets, outputs
    return loss_dict
...
def train_one_epoch(
    ...
    # TODO: какое выбрать?.. Правильно ли, что максимального размера?
    # print(f"VQA dataloader size: {len(data_loader_vqa)}. zsOD dataloader size: {len(data_loader_zsOD)}")
    if len(data_loader_vqa) >= len(data_loader_zsOD):
        bigger_data_loader = data_loader_vqa
        smaller_data_loader = data_loader_zsOD
        vqa_bigger = True
    else:
        bigger_data_loader = data_loader_zsOD
        smaller_data_loader = data_loader_vqa
        vqa_bigger = False
    # print(f"Difference in sizes: {len(bigger_data_loader) - len(smaller_data_loader)}")

    smaller_data_iter = iter(smaller_data_loader)

    num_training_steps = int(len(bigger_data_loader) * args.epochs)
    for i, bigger_data_batch_dict in enumerate(metric_logger.log_every(bigger_data_loader, print_freq, header)):
        curr_step = epoch * len(bigger_data_loader) + i

        # Возьмём батч по меньшему датасету, зацикленная итерация
        try:
            smaller_data_batch_dict = next(smaller_data_iter)
        except StopIteration:
            smaller_data_iter = iter(smaller_data_loader)
            smaller_data_batch_dict = next(smaller_data_iter)

        if vqa_bigger:
            # Тогда в бОльшем батче данные из VQA => детекция фиктивная и мы не считаем лосс на ней
            loss_dict = go_through_epoch(args, model, criterion=None, contrastive_criterion=None,
                                         qa_criterion=qa_criterion, batch=bigger_data_batch_dict, device=device)
            loss_dict.update(go_through_epoch(args, model, criterion, contrastive_criterion, qa_criterion=None,
                                              batch=smaller_data_batch_dict, device=device))
        else:
            loss_dict = go_through_epoch(args, model, criterion, contrastive_criterion, qa_criterion=None,
                                         batch=bigger_data_batch_dict, device=device)
            # Тогда в батче от меньшего датасета данные из VQA => детекция фиктивная и мы не считаем лосс на ней
            loss_dict.update(go_through_epoch(args, model, criterion=None, contrastive_criterion=None,
                                              qa_criterion=qa_criterion, batch=smaller_data_batch_dict, device=device))
...
def evaluate(
    ...
    for bigger_data_batch_dict in metric_logger.log_every(bigger_data_loader, 10, header):
        try:
            smaller_data_batch_dict = next(smaller_data_iter)
        except StopIteration:
            smaller_data_iter = iter(smaller_data_loader)
        ...
        if vqa_bigger:
            ...
            loss_dict = go_through_epoch(args, model, criterion=None, contrastive_criterion=None,
                                         qa_criterion=qa_criterion, batch=bigger_data_batch_dict, device=device)
            loss_dict_small, targets, outputs = go_through_epoch(
                args, model, criterion, contrastive_criterion, qa_criterion=None, batch=smaller_data_batch_dict,
                device=device, return_staff=True
            )
            loss_dict.update(loss_dict_small)
        else:
            loss_dict, targets, outputs = go_through_epoch(
                args, model, criterion, contrastive_criterion, qa_criterion=None, batch=bigger_data_batch_dict,
                device=device, return_staff=True
            )
            # Тогда в батче от меньшего датасета данные из VQA => детекция фиктивная и мы не считаем лосс на ней
            loss_dict.update(go_through_epoch(args, model, criterion=None, contrastive_criterion=None,
