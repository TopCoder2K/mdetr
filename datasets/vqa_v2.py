...
def build(image_set, args):
    ...
    if args.do_qa:
        # TODO: проверить, что по аргументам
        assert args.vqa_split_type is not None

        if image_set == "train":
            datasets = []
            for imset in ["train", "val"]:
                # TODO: args.gqa_split_type
                ann_file = Path(args.vqa_ann_path) / f"finetune_vqa2_{imset}.json"
                ...
                datasets.append(
                    GQAQuestionAnswering(
                        ...
                        return_tokens=True,
                        tokenizer=tokenizer,
                        ann_folder=Path(args.vqa_ann_path),
                    )
                )
            ...
            # TODO: args
            ann_file = Path(args.vqa_ann_path) / f"finetune_vqa2_minival.json"
            ...
            # TODO: args...
            ann_file = Path(args.vqa_ann_path) / f"finetune_vqa2_{image_set}.json"
        ...
        # TODO: снова проверить args и вообще нужно ли это?????
        ann_file = Path(args.gqa_ann_path) / f"_{image_set}.json"
