...
class ZSObjectDetectionBase(torchvision.datasets.VisionDataset):
    def __init__(self, root, annFile, transform=None, target_transform=None, transforms=None):
        super(ZSObjectDetectionBase, self).__init__(root, transforms, transform, target_transform)
        self.zsod = ZSOD(annFile)
        self.ids = list(sorted(self.zsod.imgs.keys()))
    ...
    def __getitem__(self, index):
        ...
        # ann_ids = zsod.get_ann_ids(img_ids=img_id)  # Вернёт пустой список, так как нет annotations
        # target = zsod.load_anns(ann_ids)  # Должна бросить RE, так как self.zsod.anns пустой, поэтому закомментил
        target = []

        path = "/".join([self.zsod.load_imgs(img_id)[0]["file_name"]])
...
class ZSObjectDetection(ZSObjectDetectionBase):
    def __init__(self, img_folder, ann_file, transforms, return_masks):
        super(ZSObjectDetection, self).__init__(img_folder, ann_file)
    ...
    def __getitem__(self, idx):
        ...
        target["file_name"] = self.zsod.imgs[image_id]["file_name"]
        target["orig_caption"] = self.zsod.imgs[image_id]["orig_caption"]
        return img, target


def build(image_set, args):
    input_folder = Path(args.fb_input_dir)
    ann_file = input_folder / "zsOD/inference_zsOD_fusion_brain.json"
    img_folder = input_folder / "zsOD/images"

    dataset = ZSObjectDetection(
        img_folder,
        ann_file,
